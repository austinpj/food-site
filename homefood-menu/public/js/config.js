/**
 * ============================================================
 *  FILL THESE TWO VALUES IN BEFORE DEPLOYING
 * ============================================================
 * 1. Go to https://supabase.com/dashboard -> your project
 * 2. Settings (gear icon) -> API
 * 3. Copy "Project URL" into SUPABASE_URL below
 * 4. Copy the "anon" "public" key into SUPABASE_ANON_KEY below
 *    (this key is SAFE to expose in frontend code — it is
 *    designed to be public and is protected by Row Level
 *    Security on the Supabase side)
 * ============================================================
 */
window.SUPABASE_URL = "https://pjpmnngilvmoctdrsytl.supabase.co";
window.SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBqcG1ubmdpbHZtb2N0ZHJzeXRsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODcxMjQwOTMsImV4cCI6MjEwMjcwMDA5M30.qCBYdbAPrt9pU0aR0-6oRqVdn-lFavsWH8BqQHrd0PQ";
